...
<body>
  ...
  <div class="navbar">
    <div id="oopppt" class="pptbook">
      <i class="fas fa-file-alt"></i>
      <span>ppts</span>
    </div>
    <div id="oopref" class="pptbook">
  ...
  <div class="container" id="oopcontainer">
    <?php
    ...
    if ($usertype == 'c') {
        ...
        if (isset($_POST['save'])) {
          ...
          if (!in_array($extension, ['zip', 'pdf', 'docx', 'pptx', 'ppt'])) {
            ...
            if (move_uploaded_file($file, $destination)) {
              $sql = "INSERT INTO `oopppt`(`module`, `teacher`, `name`) VALUES ('$modu','$teach','$filename')";
      ...
      <div class="row">
        <form action="oop.php" method="post" enctype="multipart/form-data">
          <h3>Upload File</h3>
    ...
    <table>
      ...
      <tbody>
        <?php
        $querry = mysqli_query($conn, "SELECT * FROM oopppt");
    ...
    <div hidden>
      <div id="ooprefcont">
        ...
        if ($usertype == 'c') {
          if (isset($_POST['save'])) {
            ...
            if (!in_array($extension, ['zip', 'pdf', 'docx', 'pptx', 'ppt'])) {
              ...
              if (move_uploaded_file($file, $destination)) {
                $sql = "INSERT INTO `oopref`(`name`) VALUES ('$filename')";
                if (mysqli_query($conn, $sql)) {
                  echo "<script>alert('uploaded successfully');</script>";
                }
              } else {
        ...
        <table>
          ...
          <tbody>
            <?php
            $querry = mysqli_query($conn, "SELECT * FROM oopref");
            while ($val = mysqli_fetch_array($querry)) {
            ?>
              <tr>
                <th><?php echo $val["name"]; ?></th>
  ...
  <script>
    container = document.querySelector("#oopcontainer");
    ppthtml = container.innerHTML;
    pptb = document.querySelector("#oopppt");
    pptb.addEventListener('click', function() {
      console.log("clicekd ppt");
      container.innerHTML = ppthtml;
    });

    refb = document.querySelector("#oopref");
    refb.addEventListener('click', function() {
      container.innerHTML = document.querySelector("#ooprefcont").innerHTML;
    });
  </script>
