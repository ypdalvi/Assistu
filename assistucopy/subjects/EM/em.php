...
<body>
  ...
  <div class="navbar">
    <div id="emppt" class="pptbook">
      <i class="fas fa-file-alt"></i>
      <span>ppts</span>
    </div>
    <div id="emref" class="pptbook">
  ...
  <div class="container" id="emcontainer">
    <?php
    ...
    if ($usertype == 'c') {
        ...
        if (isset($_POST['save'])) {
          ...
          if (!in_array($extension, ['zip', 'pdf', 'docx', 'pptx', 'ppt'])) {
            ...
            if (move_uploaded_file($file, $destination)) {
              $sql = "INSERT INTO `emppt`(`module`, `teacher`, `name`) VALUES ('$modu','$teach','$filename')";
      ...
      <div class="row">
        <form action="em.php" method="post" enctype="multipart/form-data">
          <h3>Upload File</h3>
    ...
    <table>
      ...
      <tbody>
        <?php
        $querry = mysqli_query($conn, "SELECT * FROM emppt");
    ...
    <div hidden>
      <div id="emrefcont">
        ...
        if ($usertype == 'c') {
          if (isset($_POST['save'])) {
            ...
            if (!in_array($extension, ['zip', 'pdf', 'docx', 'pptx', 'ppt'])) {
              ...
              if (move_uploaded_file($file, $destination)) {
                $sql = "INSERT INTO `emref`(`name`) VALUES ('$filename')";
                if (mysqli_query($conn, $sql)) {
                  echo "<script>alert('uploaded successfully');</script>";
                }
              } else {
        ...
        <table>
          ...
          <tbody>
            <?php
            $querry = mysqli_query($conn, "SELECT * FROM emref");
            while ($val = mysqli_fetch_array($querry)) {
            ?>
              <tr>
                <th><?php echo $val["name"]; ?></th>
  ...
  <script>
    container = document.querySelector("#emcontainer");
    ppthtml = container.innerHTML;
    pptb = document.querySelector("#emppt");
    pptb.addEventListener('click', function() {
      console.log("clicekd ppt");
      container.innerHTML = ppthtml;
    });

    refb = document.querySelector("#emref");
    refb.addEventListener('click', function() {
      container.innerHTML = document.querySelector("#emrefcont").innerHTML;
    });
  </script>
