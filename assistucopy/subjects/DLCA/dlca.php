...
<body>
  ...
  <div class="navbar">
    <div id="dlcappt" class="pptbook">
      <i class="fas fa-file-alt"></i>
      <span>ppts</span>
    </div>
    <div id="dlcaref" class="pptbook">
  ...
  <div class="container" id="dlcacontainer">
    <?php
    ...
    if ($usertype == 'c') {
        ...
        if (isset($_POST['save'])) {
          ...
          if (!in_array($extension, ['zip', 'pdf', 'docx', 'pptx', 'ppt'])) {
            ...
            if (move_uploaded_file($file, $destination)) {
              $sql = "INSERT INTO `dlcappt`(`module`, `teacher`, `name`) VALUES ('$modu','$teach','$filename')";
      ...
      <div class="row">
        <form action="dlca.php" method="post" enctype="multipart/form-data">
          <h3>Upload File</h3>
    ...
    <table>
      ...
      <tbody>
        <?php
        $querry = mysqli_query($conn, "SELECT * FROM dlcappt");
    ...
    <div hidden>
      <div id="dlcarefcont">
        ...
        if ($usertype == 'c') {
          if (isset($_POST['save'])) {
            ...
            if (!in_array($extension, ['zip', 'pdf', 'docx', 'pptx', 'ppt'])) {
              ...
              if (move_uploaded_file($file, $destination)) {
                $sql = "INSERT INTO `dlcaref`(`name`) VALUES ('$filename')";
                if (mysqli_query($conn, $sql)) {
                  echo "<script>alert('uploaded successfully');</script>";
                }
              } else {
        ...
        <table>
          ...
          <tbody>
            <?php
            $querry = mysqli_query($conn, "SELECT * FROM dlcaref");
            while ($val = mysqli_fetch_array($querry)) {
            ?>
              <tr>
                <th><?php echo $val["name"]; ?></th>
  ...
  <script>
    container = document.querySelector("#dlcacontainer");
    ppthtml = container.innerHTML;
    pptb = document.querySelector("#dlcappt");
    pptb.addEventListener('click', function() {
      console.log("clicekd ppt");
      container.innerHTML = ppthtml;
    });

    refb = document.querySelector("#dlcaref");
    refb.addEventListener('click', function() {
      container.innerHTML = document.querySelector("#dlcarefcont").innerHTML;
    });
  </script>
